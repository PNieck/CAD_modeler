#include "CAD_modeler/model/systems/trimmedC0PatchesRenderSystem.hpp"
...
void TrimmedC0PatchesRenderSystem::RegisterSystem(Coordinator &coordinator)
{
    coordinator.RegisterSystem<TrimmedC0PatchesRenderSystem>();

    coordinator.RegisterComponent<DrawTrimmed>();

    coordinator.RegisterRequiredComponent<TrimmedC0PatchesRenderSystem, C0Patches>();
    coordinator.RegisterRequiredComponent<TrimmedC0PatchesRenderSystem, PatchesDensity>();
    coordinator.RegisterRequiredComponent<TrimmedC0PatchesRenderSystem, Mesh>();
    coordinator.RegisterRequiredComponent<TrimmedC0PatchesRenderSystem, DrawTrimmed>();
    coordinator.RegisterRequiredComponent<TrimmedC0PatchesRenderSystem, UvVisualization>();
}


void TrimmedC0PatchesRenderSystem::Render(const alg::Mat4x4 &cameraMtx)
{
    if (entities.empty()) {
        return;
    }
    ...
    auto const& shader = ShaderRepository::GetInstance().GetTrimmedBezierSurfaceShader();
